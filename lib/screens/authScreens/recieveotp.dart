...
import 'package:theharmony/models/RecieveOtpModel.dart';
import 'package:theharmony/models/submitModel.dart';
...
class RecieveOtp extends StatefulWidget {
  ...
  const RecieveOtp({Key? key, required this.userNumber}) : super(key: key);
...
class _RecieveOtpState extends State<RecieveOtp> {
  ...
  List<String> RecieveOtpImageList = [
    'assets/images/nut03.png',
    'assets/images/dairy03.png',
    'assets/images/nut04.png',
                            ...
                            child: GestureDetector(
                              onTap: () async {
                                ...
                                if(otpController.text.isNotEmpty){
                                  await _reciveOtp(otpController.text);
                                }
                              },
                              child: Container(
                                width: 0.9.sw,
  ...
  _reciveOtp(otp) async {
    try {
      print('inside submit otp api');
      ...
      var response =
      await http.post(Uri.parse(recieveOtp), body: params, headers: headers);
      if (kDebugMode) {
        print(response.body);
      }
      if (response.statusCode == 200) {
        ...
        if(json["status code"].toString() == '200'){
          ...
          //var data = SubmitModel.fromJson(json);
          token = json["access_token"].toString();
          print('print user id : ');

          var data = RecieveOtpModel.fromJson(json);
          var temp = data.userId;

          mainUserId = temp[0].id;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(backgroundColor: Colors.red,content: Text("message")));
      }
    } catch (e) {
      print(e);
      setState(() {
